...
def validate_summary_embeddings():
    ...
    for doc in processed_docs:
        ...
        summary_chunks = Embedding.objects.filter(
            document=doc,
            metadata__is_document_summary=True
        )
    ...
    print(f"Documents WITH Summary:    {docs_with_summary} ({docs_with_summary/total_docs*100:.1f}%)")
    print(f"Documents WITHOUT Summary: {docs_without_summary} ({docs_without_summary/total_docs*100:.1f}%)")
    print()
    ...
    if compliance_pct == 100:
        ...
        print(f"⚠️  PRD v2.1 FR-1.3 COMPLIANCE: PARTIAL ({compliance_pct:.1f}% coverage)")
    else:
