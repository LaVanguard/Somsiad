...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--all", action="store_true", help="Process all unprocessed documents"
        )
        parser.add_argument(
            "--document-id", type=int, help="Process specific document by ID"
        )
        parser.add_argument(
            "--reprocess",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if options["all"]:
            self.stdout.write(
                self.style.WARNING("Processing all unprocessed documents...")
            )
        ...
        if options["document_id"]:
            try:
                ...
                raise CommandError(
                    f"Document with ID {options['document_id']} not found"
                )
            ...
            if success:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"[SUCCESS] Successfully processed: {document.title}"
                    )
                )
            else:
                self.stdout.write(
                    self.style.ERROR(f"[ERROR] Failed to process: {document.title}")
                )
            return
