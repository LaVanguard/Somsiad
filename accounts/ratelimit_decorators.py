...
def ratelimit_login(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if getattr(request, "limited", False):
            logger.warning(
                f"Rate limit exceeded for login from IP: {request.META.get('REMOTE_ADDR')}"
            )
            return HttpResponse(
...
def ratelimit_signup(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if getattr(request, "limited", False):
            logger.warning(
                f"Rate limit exceeded for signup from IP: {request.META.get('REMOTE_ADDR')}"
            )
            return HttpResponse(
...
def ratelimit_rag_query(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if getattr(request, "limited", False):
            ...
            logger.warning(
                f"Rate limit exceeded for RAG query. User: {user_id}, IP: {request.META.get('REMOTE_ADDR')}"
            )
            return HttpResponse(
...
def ratelimit_document_upload(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if getattr(request, "limited", False):
            logger.warning(
                f"Rate limit exceeded for document upload. User: {request.user.id}"
            )
            return HttpResponse(
...
def ratelimit_document_process(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if getattr(request, "limited", False):
            logger.warning(
                f"Rate limit exceeded for document processing. User: {request.user.id}"
            )
            return HttpResponse(
